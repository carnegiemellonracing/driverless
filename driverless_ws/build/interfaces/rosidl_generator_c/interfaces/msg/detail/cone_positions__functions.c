  ...
  interfaces__msg__ConePositions__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    interfaces__msg__ConePositions * data =
      (interfaces__msg__ConePositions *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!interfaces__msg__ConePositions__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          interfaces__msg__ConePositions__fini(&output->data[i]);
        }
        return false;
      }
    }
