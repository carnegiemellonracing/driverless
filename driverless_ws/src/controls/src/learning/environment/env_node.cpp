...
namespace controls {
    ...
    namespace env {
        ...
        EnvNode::EnvNode(std::map<std::string, std::string> config_dict, std::array<float, 4> initialState)
            : 
        ...
        void EnvNode::update_visible_indices() {
        ...
        std::vector<glm::fvec2> EnvNode::arc_segment(float radius, glm::fvec2 start_pos, float start_heading, float end_heading) {
        ...
        EnvNode::SplineAndCones EnvNode::arc_segment_with_cones(float radius, glm::fvec2 start_pos, float start_heading, float end_heading) {
         ...
         */
        std::vector<glm::fvec2> EnvNode::straight_segment(glm::fvec2 start, float length, float heading) {
            std::vector<glm::fvec2> result;
        ...
        EnvNode::SplineAndCones EnvNode::straight_segment_with_cones(glm::fvec2 start, float length, float heading) {
        ...
        std::array<float, 4> EnvNode::getWorldState() {
            return m_world_state;
        }

        void EnvNode::on_sim(Action &control_action) {
            Action adj_msg = control_action;
        ...
        void EnvNode::publish_track(TrackState &s) {
        ...
        void EnvNode::publish_twist(TrackState &s) {
            s.twist = m_world_state[3];
        }
        
        std::deque<Segment> EnvNode::parse_segments_specification(std::string track_specifications_path)
        {
...
int main(int argc, char* argv[]){
    ...
    std::array<float, 4> initialState = {0.0186932, 0.00213094, 0.00272238, 0.168475};

    controls::env::EnvNode minusRosTestNode = controls::env::EnvNode(config_dict, initialState);

    controls::env::Action a = {
        .torque_fl = 3.83714,
        .torque_fr = 3.83714,
        .torque_rl = 0,
        .torque_rr = 0,
