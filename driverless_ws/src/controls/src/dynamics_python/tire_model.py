...
def Fx0(p, longslip, Fz, pressure, inclinagl):
    ...
    Fz0_ = p['FNOMIN'].value * p['LFZO'].value
    dfz = (Fz - Fz0_) / Fz0_
    pi0 = p['NOMPRES'].value
    dpi = (pressure - pi0) / pi0
    Cx = p['PCX1'].value * p['LCX'].value
    mux = p['LMUX'].value * (p['PDX1'].value + p['PDX2'].value * dfz) * (1 + p['PPX3'].value * dpi + p['PPX4'].value * dpi**2) * (1 - p['PDX3'].value * inclinagl**2)
    Dx = mux * Fz * ZETA1
    SHx = p['LHX'].value * (p['PHX1'].value + p['PHX2'].value * dfz)
    AMU = 10
    LMUX_ = AMU * p['LMUX'].value / (1 + (AMU - 1) * p['LMUX'].value)
    SVx = ZETA1 * LMUX_ * p['LVX'].value * Fz * (p['PVX1'].value + p['PVX2'].value * dfz)
    kappax = longslip + SHx
    kappaxSgn = np.sign(kappax)
    Ex = p['LEX'].value * (p['PEX1'].value + p['PEX2'].value * dfz + p['PEX3'].value * dfz**2) * (1 - p['PEX4'].value * kappaxSgn)
    Kxk = p['LKX'].value * Fz * (p['PKX1'].value + p['PKX2'].value * dfz) * np.exp(p['PKX3'].value * dfz) * (1 + p['PPX1'].value * dpi + p['PPX2'].value * dpi**2)
...
def Fx(p, longslip, slipangl, Fz, pressure, inclinagl):
    ...
    Cxa = p['RCX1'].value
    Exa = p['REX1'].value + p['REX2'].value * dfz
    SHxa = p['RHX1'].value
    alphaS = alphaAst + SHxa
    Bxa = p['LXAL'].value * (p['RBX1'].value + p['RBX3'].value * gammaAst**2) * np.cos(np.arctan(p['RBX2'].value * longslip))
...
def Fy0(p, slipangl, Fz, pressure, inclinagl):
    ...
    dpi = (pressure - pi0) / pi0
    dpi2 = dpi**2
    Cy = p['LCY'].value * p['PCY1'].value
    muy = (p['PDY1'].value + p['PDY2'].value * dfz) * (1 + p['PPY3'].value * dpi + p['PPY4'].value * dpi2) * (1 - p['PDY3'].value * gammaAst2) * p['LMUY'].value
    Dy = muy * Fz * ZETA2
    Kya = p['PKY1'].value * Fz0_ * (1 + p['PPY1'].value * dpi) * (1 - p['PKY3'].value * np.abs(gammaAst)) * np.sin(p['PKY4'].value * np.arctan(Fz / Fz0_ / ((p['PKY2'].value + p['PKY5'].value * gammaAst2) * (1 + p['PPY2'].value * dpi)))) * p['LKY'].value * ZETA3
    ...
    Kya_ = Kya + eps_kya
    SVyg = ZETA2 * p['LKYC'].value * p['LMUY'].value * Fz * (p['PVY3'].value + p['PVY4'].value * dfz) * gammaAst
    Kyg0 = Fz * (p['PKY6'].value + p['PKY7'].value * dfz) * (1 + p['PPY5'].value * dpi) * p['LKYC'].value
    SVy = ZETA2 * p['LMUY'].value * p['LVY'].value * Fz * (p['PVY1'].value + p['PVY2'].value * dfz) + SVyg
    SHy = p['LHY'].value * (p['PHY1'].value + p['PHY2'].value * dfz) + (Kyg0 * gammaAst - SVyg) / Kya_ * ZETA0 + ZETA4 - 1
    alphay = slipangl + SHy
    alphaySgn = np.sign(slipangl)
    Ey = (p['PEY1'].value + p['PEY2'].value * dfz) * (1 + p['PEY5'].value * gammaAst**2 - (p['PEY3'].value + p['PEY4'].value * gammaAst) * alphaySgn) * p['LEY'].value
    # signCy = np.sign(Cy)
    # signCy[signCy == 0] = 1
...
def Fy(p, longslip, slipangl, Fz, pressure, inclinagl):
    ...
    alphaAst = np.tan(slipangl)
    Byk = p['LYKA'].value * (p['RBY1'].value + p['RBY4'].value * gammaAst**2) * np.cos(np.arctan(p['RBY2'].value * (alphaAst - p['RBY3'].value)))
    Cyk = p['RCY1'].value
    DVyk = 1 * muy * Fz * (p['RVY1'].value + p['RVY2'].value * dfz + p['RVY3'].value * gammaAst) * np.cos(np.arctan(p['RVY4'].value * alphaAst))
    Eyk = p['REY1'].value + p['REY2'].value * dfz
    SHyk = p['RHY1'].value + p['RHY2'].value * dfz
    SVyk = p['LVYKA'].value * DVyk * np.sin(p['RVY5'].value * np.arctan(p['RVY6'].value * longslip))
    ...
    Fy = Gyk * fy0 + SVyk
    return Fy, muy, Gyk, Byk, Eyk, fy0, gammaAst

def get_forces(longslip, slipangl, Fz, pressure, inclinagl):
    tire = TireParameters()
    p = tire.__dict__
    fx, _, _, _, _= Fx(p, longslip, slipangl, Fz, pressure, inclinagl)
    fy, _, _, _, _, _, _ = Fy(p, longslip, slipangl, Fz, pressure, inclinagl)
    return fx, fy