...
def kinematic_swangle(speed, swangle):
    return swangle / (1 + understeer_slope * speed)

def angular_speed(speed, kinematic_swangle_):
...
def centripedal_accel(speed, swangle):
    if swangle == 0:
        return 0

    kinematic_swangle_ = kinematic_swangle(speed, swangle)
    back_whl_to_center_of_rot = (cg_to_front + cg_to_rear) / math.tan(kinematic_swangle_)
    return math.copysign(
...
def slip_angle(kinematic_swangle):
...
def dynamics(state, action):
    ...
    kinematic_swangle_ = kinematic_swangle(speed, swangle)
    slip_angle_ = slip_angle(kinematic_swangle_)
    ...
    next_speed_raw = speed + (
        ...
        (whl_radius * car_mass) - rolling_drag / car_mass
    ) * timestep
    ...
    angular_speed_ = angular_speed(dist_avg_speed, kinematic_swangle_)
