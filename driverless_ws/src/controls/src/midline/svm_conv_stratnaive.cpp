    ...
    for (size_t row = 0; row < rows; ++row) {
        for (size_t col = 0; col < cols; ++col) {
            ...
            if (abs(val - 1) < 0.1) {
                ...
                else if (val != nodePredictor({xx[row][std::max<size_t>(col - 1, 0)], yy[row][std::max<size_t>(col - 1, 0)]}, model))
                    boundary_points.emplace_back(x, y);
                else if (val != nodePredictor({xx[std::min(row + 1, rows - 1)][col], yy[std::min(row + 1, rows - 1)][col]}, model))
                    boundary_points.emplace_back(x, y);
                else if (val != nodePredictor({xx[std::max<size_t>(row - 1, 0)][col], yy[std::max<size_t>(row - 1, 0)][col]}, model))
                    boundary_points.emplace_back(x, y);
            }
        }
    }
