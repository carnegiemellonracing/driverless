                            ...
                            const svm_model *model)
{
    ...
    for (size_t row = 0; row < rows; ++row)
    {
        ...
        if (left_label == right_label)
        {
           ...
           {
               double label = nodePredictor({xx[row][col], yy[row][col]}, model);
               if (label != left_label)
               {
                    if(abs(label - 1) < 0.1)
                        boundary_points.emplace(xx[row][col], yy[row][col]);
                    else
                        boundary_points.emplace(xx[row][col-1], yy[row][col-1]);
                   left_label = label;
               }
           }
        }
           ...
           for (size_t row = 0; row < rows; ++row)
           {
               ...
               if (label != top_label)
               {
                    if(abs(label - 1) < 0.1)
                        ...
                        boundary_points.emplace(xx[row-1][col], yy[row-1][col]);
