...
{
    std::cout << "Starting naive model" << std::endl;
    auto points_brute_force = controls::midline::svm_naive::boundaryDetection(xx,yy,model);
    std::cout << "Strating strat3 model" << std::endl;
    auto points_strat3 = controls::midline::svm_strat3::boundaryDetection(xx,yy,model);
    //auto points_strat4 = controls::midline::svm_strat4::boundaryDetection(xx,yy,model);


    paranoid_assert(points_brute_force.size() == points_strat3.size());

    std::cout << "Strating sorting naive model" << std::endl;
    //points_brute_force = sortBoundaryPoints(points_brute_force);
    std::sort(points_brute_force.begin(),points_brute_force.end(), [](std::pair<double, double> a, std::pair<double, double> b)
    {
        if(a.first > b.first)
            return true;
    ...
    std::cout << "Strating sorting strat3 model"<< std::endl;
    std::sort(points_strat3.begin(),points_strat3.end(), [](std::pair<double, double> a, std::pair<double, double> b)
    {
        if(a.first > b.first)
            return true;
        else if (a.first == b.first)
    ...
    //points_strat3 = sortBoundaryPoints(points_strat3);
    //points_strat4 = sortBoundaryPoints(points_strat4);
    
    //paranoid_assert(points_brute_force.size() == points_strat4.size());
    for (size_t i = 0; i < points_brute_force.size(); ++i)
    {
        std::cout << "In loop line" << i << std::endl;
        paranoid_assert(points_brute_force[i].first == points_strat3[i].first);
        paranoid_assert(points_brute_force[i].second == points_strat3[i].second);
        // paranoid_assert(points_brute_force[i].first == points_strat4[i].first);
        // paranoid_assert(points_brute_force[i].second == points_strat4[i].second);
    }
    std::cout << "Finished boundary Detection code"<< std::endl;
}
