...
namespace controls {
    namespace tests {
        ...
        void detect_all_collisions() {
            ...
            std::string collision_log_path = getenv("DRIVERLESS") + g_config_dict["collision_logs"];
        ...
        TestNode::TestNode(std::map<std::string, std::string> config_dict)
            : Node{"test_node"},
              ...
              m_all_segments{parse_segments_specification(getenv("DRIVERLESS") + m_config_dict["track_specs"])},

              m_lookahead{std::stof(m_config_dict["look_ahead"])},
              m_lookahead_squared{m_lookahead * m_lookahead},

              m_log_file{getenv("DRIVERLESS") + m_config_dict["track_logs"], std::ios_base::trunc},
