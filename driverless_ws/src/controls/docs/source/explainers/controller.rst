...
We found that using this same solution for GPU-based parameters likely increases the latency of the controller,
but more ablation testing is necessary.

If we can successfully implement run-time parameters for GPU-based parameters, we can effectively have build-free
track tests. 