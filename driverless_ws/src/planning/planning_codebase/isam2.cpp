...
public:
    ...
    void step(gtsam::Pose2 global_odom, std::vector<Point2> &cone_obs) {
        ...
        if (x==0) {
            ...
            Pose2 prev_pos = isam.calculateEstimate(X(x - 1)).cast<Pose2>();
        ...
        graph.resize(0);
        values.clear();
        Pose2 robot_est = isam.calculateEstimate(X(x)).cast<Pose2>();
        ...
        robot_est = isam.estimate(); //extract robot variable
        //isam.calculateEstimate(X(x)).cast<gtsam::Pose2>();
        x++;

        landmark_est.clear();
        for (int i = 0; i < n_landmarks; i++) {
            landmark_est.push_back(isam.calculateEstimate(L(i)).cast<gtsam::Point2>());
        }
