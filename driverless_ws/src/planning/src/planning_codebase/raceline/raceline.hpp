...
public:
    ...
    // first row is unit, second row is x
    ...
    // first row is unit, second row is y
    ...
    polynomial get_SplPoly_x() { 
        return spl_poly_x;
    } // original polynomial

    polynomial get_SplPoly_y() { 
        return spl_poly_y;
    } // original polynomial
    
    void set_SplPoly(polynomial p_x, polynomial p_y){ // setting polynomials
        spl_poly_x.deg = p_x.deg;
        spl_poly_x.nums = p_x.nums;
        first_der_x = polyder(spl_poly_x);
        second_der_x = polyder(first_der_x);

        spl_poly_y.deg = p_y.deg;
        spl_poly_y.nums = p_y.nums;
        first_der_y = polyder(spl_poly_y);
        second_der_y = polyder(first_der_y);
    }


    polynomial get_first_der_x();
    polynomial get_first_der_y();
    polynomial get_second_der_x();
    polynomial get_second_der_y();

    Eigen::MatrixXd get_points_x();
    Eigen::MatrixXd get_points_y();
    ...
    // second spline function is main one to useju
    Spline(polynomial poly_x, polynomial poly_y, Eigen::VectorXd points_x, Eigen::VectorXd points_y);
...
double arclength(polynomial fder_x, polynomial fder_y, double x0, double x1);
