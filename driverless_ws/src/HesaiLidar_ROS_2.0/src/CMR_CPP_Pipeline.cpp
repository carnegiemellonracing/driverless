 ...
 * @param left_hbound: Horizontal left (min) boundary for manual filtering (Note: should be positive)
 * @param right_hbound: Horizontal right (max) boundary for manual filtering (Note: should be positive)
...
inline PointCloud<PointXYZ> GraceAndConrad(PointCloud<PointXYZ> cloud, double alpha,
                                           int num_bins, double height_threshold, double right_hbound, double left_hbound) {
    ...
    for (int i = 0; i < csize; i++) {
        ...
        // Added manual filtering option, keeps points within left and right horizontal bound
        if (rd.radius < radius_max && -left_hbound < rd.y < right_hbound) {
...
/**
 * Function that returns indices of points in cloud that are within distance epsilon of input point
 * @include banana
 * @param cloud: The pointclooud
 * @param point: The point to check distance to
 * @param epsilon: Max distance from point
 * @return Indices of points within radius epsilon of input point (vector<int>)
 */
...
/**
 * Function that expands the cluster by checking neighbors and assigning them as needed.
 * @param cloud: The point cloud being received
 * @param visited: Colections of booleans for each point checking if they ve been visited
 * @param cluster: Collection of indices of the cluster points
 * @param point_idx: The center of the current cluster
 * @param neighbors: Stores number of neighbors for each cluster point
 * @param epsilon: Minimum radius for things to be included the cluster.
 * @param cluster_id: The id of the current cluster
 * @param min_points: Minimum number of points to be considered a cluster
 * @return void
 */
 ...
 * @param clusters: The clusters to compute centroids for
...
/**
/**
 * DBSCAN on a PointCloud of PointXYZ
 * @param cloud: The input point cloud
...
/**
 * Secondary DBSCAN to filter out extraneous clusters
 * @param cloud: The input point cloud
 * @param epsilon: The minimum distance between points
 * @param min_points: The minimum number of points to form a cluster
 * @return Filtered
...
/**
 * Function to run full CPP pipline
 * @param cloud: The input point cloud
 * @param alpha: The size of each segment (radians)
 * @param num_bins: The number of bins per segment
 * @param height_threshold: Keep all points this distance above the best fit line
 * @param epsilon: The minimum distance between points
 * @param min_points: The minimum number of points to form a cluster
 * @param epsilon2: The minimum distance between points for the filtering DBSCAN
 * @param min_points2: The minimum number of points to form a cluster for the filtering DBSCAN
