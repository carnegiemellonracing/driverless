...
//------------------------------------------------------------------------------
class GpsImu9004 final : public SpecializedDataContainer
{
...
public:
    constexpr static const char* const containerType{"sdk.specialcontainer.gpsimu9004"};

    static constexpr uint64_t getClassIdHashStatic() { return hash(containerType); }

public:
    GpsImu9004();
    ~GpsImu9004() override = default;
...
public:
    ...
    //========================================
    //!\brief Get the mounting yaw angle of the GPS/IMU device in [rad], between[-pi;pi].
    //!\return The mounting yaw.
    //----------------------------------------
    double getMountingAngleYaw() const { return m_mountingAngleYaw; }

    //========================================
    //!\brief Get the mounting pitch angle of the GPS/IMU device in [rad], between[-pi;pi].
    //!\return The mounting pitch.
    //----------------------------------------
    double getMountingAnglePitch() const { return m_mountingAnglePitch; }

    //========================================
    //!\brief Get the mounting roll angle of the GPS/IMU device in [rad], between[-pi;pi].
    //!\return The mounting roll.
    //----------------------------------------
    double getMountingAngleRoll() const { return m_mountingAngleRoll; }

    //========================================
    //!\brief Get the x mounting offset of the GPS/IMU device in [m].
    //!\return The x mounting offset.
    //----------------------------------------
    double getMountingOffsetX() const { return m_mountingOffsetX; }

    //========================================
    //!\brief Get the y mounting offset of the GPS/IMU device in [m].
    //!\return The y mounting offset.
    //----------------------------------------
    double getMountingOffsetY() const { return m_mountingOffsetY; }

    //========================================
    //!\brief Get the z mounting offset of the GPS/IMU device in [m].
    //!\return The z mounting offset.
    //----------------------------------------
    double getMountingOffsetZ() const { return m_mountingOffsetZ; }
...
public:
    ...
    //========================================
    //!\brief Set the cross acceleration [m/s^2].
    //!\param[in] crossAcceleration  The cross acceleration.
    //----------------------------------------
    ...
    //========================================
    //!\brief Set the cross acceleration uncertainty [m/s^2].
    //!\param[in] crossAccSigma  The cross acceleration uncertainty.
    //----------------------------------------
    ...
    //========================================
    //!\brief Set the longitudinal acceleration  [m/s^2].
    //!\param[in] longAcc  The longitudinal acceleration.
    //----------------------------------------
    ...
    //========================================
    //!\brief Set the longitudinal acceleration uncertainty [m/s^2].
    //!\param[in] longAccSigma  The longitudinal acceleration uncertainty.
    //----------------------------------------
    ...
    //========================================
    //!\brief Set the vertical acceleration [m/s^2].
    //!\param[in] vertAcc The vertical acceleration.
    //----------------------------------------
    ...
    //========================================
    //!\brief Set the vertical acceleration uncertainty [m/s^2].
    //!\param[in] vertAccSigma  The vertical acceleration uncertainty.
    //----------------------------------------
    ...
    //!\param[in] velocityNorthSigma  The velocity north uncertainty.
    //----------------------------------------
    ...
    //!\param[in] velocityWestSigma  The velocity west uncertainty.
    //----------------------------------------
    ...
    //!\param[in] velocityUpSigma  The velocity up uncertainty.
    //----------------------------------------
    ...
    //!\param[in] velocityXSigma  The velocity uncertainty in x direction.
    //----------------------------------------
    ...
    //!\param[in] velocityYSigma  The velocity uncertainty in y direction.
    //----------------------------------------
    ...
    //!\param[in] velocityZSigma  The velocity uncertainty in z direction.
    //----------------------------------------
    ...
    //!\param[in] rollRateSigma  The roll rate uncertainty.
    //----------------------------------------
    ...
    //!\param[in] yawRateSigma  The yaw rate uncertainty.
    //----------------------------------------
    ...
    //!\param[in] pitchRateSigma  The pitch rate uncertainty.
    //----------------------------------------
    ...
    //!\param[in] gpsDilutionOfPrecisionX  The dilution of precision in x direction.
    //----------------------------------------
    ...
    //!\param[in] gpsDilutionOfPrecisionY  The dilution of precision in y direction.
    //----------------------------------------
    ...
    //!\param[in] gpsDilutionOfPrecisionHorizontal  The dilution of precision in horizontal direction.
    //----------------------------------------
    ...
    //!\param[in] gpsDilutionOfPrecisionVertical  The dilution of precision in vertical direction.
    //----------------------------------------
    ...
    //!\param[in] gpsDilutionOfPrecisionPosition  The dilution of precision in position.
    //----------------------------------------
    ...
    //!\param[in] gpsDilutionOfPrecisionTime  The dilution of precision in time.
    //----------------------------------------
    ...
    //!\param[in] gpsDilutionOfPrecisionGeometric  The dilution of precision in geometric.
    //----------------------------------------
    ...
    //========================================
    //!\brief Set the quality flag of the GPS/IMU measurements.
...
protected:
    ...
    Timestamp m_timestamp{}; //!< Timestamps of this measurement.
    ...
    double m_mountingAngleYaw{
        NaN_double}; //!<  Mounting yaw of measurement point relative to vehicle body coordinate system in [rad], between [-pi;pi]. Rotation order z,y',x''.
    double m_mountingAnglePitch{
        NaN_double}; //!< Mounting pitch of measurement point relative to vehicle body coordinate system in [rad], between [-pi/2;pi/2]. Rotation order z,y',x''.
    double m_mountingAngleRoll{
        NaN_double}; //!<  Mounting roll of measurement point relative to vehicle body coordinate system in [rad], between [-pi;pi]. Rotation order z,y',x''.
    double m_mountingOffsetX{
        NaN_double}; //!< Mounting position x of measurement point relative to vehicle body coordinate system in [m].
    double m_mountingOffsetY{
        NaN_double}; //!< Mounting position y of measurement point relative to vehicle body coordinate system in [m].
    double m_mountingOffsetZ{
        NaN_double}; //!< Mounting position z of measurement point relative to vehicle body coordinate system in [m].

    double m_reserved1{NaN_double}; //!< Reserved for future use.
    double m_reserved2{NaN_double}; //!< Reserved for future use.
    double m_reserved3{NaN_double}; //!< Reserved for future use.

}; // GpsImu9004

//==============================================================================

bool operator==(const GpsImu9004& lhs, const GpsImu9004& rhs);
bool operator!=(const GpsImu9004& lhs, const GpsImu9004& rhs);
