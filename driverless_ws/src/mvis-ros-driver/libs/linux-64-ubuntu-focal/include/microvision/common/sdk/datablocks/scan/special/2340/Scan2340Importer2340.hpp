...
//------------------------------------------------------------------------------
template<>
class Importer<Scan2340, DataTypeId::DataType_Scan2340>
  : public RegisteredImporter<Scan2340, DataTypeId::DataType_Scan2340>
{
public:
    //========================================
...
private:
    ...
    //----------------------------------------
    static bool deserialize(std::istream& is, ScannerInfoIn2340& c);
    ...
    //!\return \c True if deserialization succeeded, otherwise: \c false
    //----------------------------------------
    static bool deserialize(std::istream& is, ScanPointIn2340& c);

    //========================================
    //!\brief Convert from byte stream (deserialization).
    //!\param[in]      is              Input data stream.
    //!\param[in, out] c               Data container.
    //!\param[in]      nbOfScanPoints  Length of the ScanPointInfoList.
    //!\return \c True if deserialization succeeded, otherwise: \c false
    //----------------------------------------
    static bool deserialize(std::istream& is, ScanPointInfoListIn2340& c, const uint32_t nbOfScanPoints);

}; // Scan2340Importer2340

//==============================================================================

using Scan2340Importer2340 = Importer<microvision::common::sdk::Scan2340, DataTypeId::DataType_Scan2340>;
