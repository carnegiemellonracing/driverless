...
public:
    ...
    //!
    //! It is an ComparableUnconvertable so it cannot be
    //! mixed up with other integer types by accident.
    //----------------------------------------
...
public:
    ...
    Version(const MajorVersion majorVersion  = MajorVersion(),
            ...
            const std::string& versionExtra  = std::string(),
            const std::string& versionString = std::string());
...
public:
    ...
    //!
    //! The value will be calculated as MMmmRRPP,
