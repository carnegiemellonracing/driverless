...
#include <microvision/common/sdk/datablocks/ldmiRaw/special/LdmiRawStaticInfoIn2352.hpp>
#include <microvision/common/sdk/datablocks/ldmiRaw/special/LdmiRawFrameHeaderIn2352.hpp>
#include <microvision/common/sdk/datablocks/ldmiRaw/special/LdmiRawFrameRowIn2352.hpp>
#include <microvision/common/sdk/datablocks/ldmiRaw/special/LdmiRawFrameFooterIn2352.hpp>
...
//------------------------------------------------------------------------------
class LdmiRawFrame2352 final : public SpecializedDataContainer
{
public:
...
public:
    ...
    //----------------------------------------
    static constexpr const char* containerType{"sdk.specialcontainer.ldmirawframe2352"};
...
public:
    ...
    //----------------------------------------
    using RowsType = std::array<LdmiRawFrameRowIn2352, LdmiRawFrameRowIn2352::maxNumberOfRows>;
...
public:
    ...
    //----------------------------------------
    ~LdmiRawFrame2352() override = default;

public:
    //========================================
    //! \brief Compares two ldmi raw frames for equality.
    //! \param[in] lhs  Ldmi raw frame 2352.
    //! \param[in] rhs  Ldmi raw frame 2352.
    //! \returns Either \c true if both frames are equal or otherwise \c false.
    //----------------------------------------
    friend bool operator==(const LdmiRawFrame2352& lhs, const LdmiRawFrame2352& rhs);
...
public: // getter
    ...
    //----------------------------------------
    const LdmiRawStaticInfoIn2352& getStaticInfo() const;

    //========================================
    //! \brief Get ldmi raw frame header.
    //! \returns Ldmi frame header.
    //----------------------------------------
    const LdmiRawFrameHeaderIn2352& getHeader() const;
    ...
    //----------------------------------------
    const LdmiRawFrameFooterIn2352& getFooter() const;
...
public: // setter
    ...
    //----------------------------------------
    void setStaticInfo(const LdmiRawStaticInfoIn2352& config);

    //========================================
    //! \brief Set ldmi raw frame header.
    //! \param[in] header  Ldmi raw frame header.
    //----------------------------------------
    void setHeader(const LdmiRawFrameHeaderIn2352& header);

    //========================================
    //! \brief Set ldmi raw frame row.
    //! \note Row will set by row id as index of array.
    //! \param[in] row  Ldmi raw frame row.
    //! \returns Either \c true if row is not set or out of range, otherwise \c false.
    //----------------------------------------
    bool setRow(const LdmiRawFrameRowIn2352& row);

    //========================================
    //! \brief Set ldmi raw frame footer package.
    //! \param[in] footer  Ldmi raw frame footer.
    //----------------------------------------
    void setFooter(const LdmiRawFrameFooterIn2352 footer);
...
private:
    ...
    //----------------------------------------
    LdmiRawStaticInfoIn2352 m_staticInfo;

    //========================================
    //! \brief Ldmi frame header.
    //----------------------------------------
    LdmiRawFrameHeaderIn2352 m_header;
    ...
    //----------------------------------------
    LdmiRawFrameFooterIn2352 m_footer;
};

//==============================================================================
//! \brief Nullable LdmiRawFrame2352 pointer.
//------------------------------------------------------------------------------
using LdmiRawFrame2352Ptr = std::shared_ptr<LdmiRawFrame2352>;
