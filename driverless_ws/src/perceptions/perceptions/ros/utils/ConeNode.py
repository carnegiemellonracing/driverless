...
from interfaces.msg import ConeArray, ConeArrayWithOdom
...
class ConeNode(Node):
    ...
    def __init__(self, merger: Merger, debug=True):
        ...
        # self.create_subscription(ConeArray, LIDAR_CONE_TOPIC, self.lidar_cone_callback,
        #                                           qos_profile=BEST_EFFORT_QOS_PROFILE)
        self.create_subscription(ConeArrayWithOdom, LIDAR_CONE_TOPIC, self.lidar_cone_callback,
                                                    qos_profile=BEST_EFFORT_QOS_PROFILE)

        # initialize cone publisher
        self.publish_timer = self.create_timer(1/PUBLISH_FPS, self.publish_cones)
        self.cone_publisher = self.create_publisher(ConeArrayWithOdom, PERC_CONE_TOPIC,
    ...
    def yolov5_zed_cone_callback(self, msg):
        ...
        self.get_logger().info(f"yolo zed callback\n")
    ...
    def yolov5_zed2_cone_callback(self, msg):
        ...
        self.get_logger().info(f"yolo zed2 callback\n")
        cones = conv.msg_to_cones(msg)
    ...
    def lidar_cone_callback(self, msg):
        ...
        cone_array_msg = msg.cone_array
        self.data_times[PipelineType.LIDAR] = Time.from_msg(cone_array_msg.orig_data_stamp)
        # self.get_logger().info(f"lidar_cone_callback{self.data_times[PipelineType.LIDAR]}")
        self.cur_msg = msg

        cones = conv.msg_to_cones(cone_array_msg)
    ...
    def publish_cones(self):
        ...
        self.cur_msg.cone_array = msg
        # self.get_logger().info(f"publish_cones{self.data_times[PipelineType.LIDAR]}")

        data_time = self.flush_and_get_data_times()
        msg.orig_data_stamp = data_time.to_msg()

        self.cone_publisher.publish(self.cur_msg)
        self.cur_msg = None
